...
_ROUGE_PATH = '/home/zixuandeng_alan/Dyle/ROUGE/RELEASE-1.5.5'
rouge_diff_thresh = 0
...
mode = "train"
# Change to own path
path = '/home/zixuandeng_alan/Dyle/QMSum/ALL/jsonl/{}.jsonl'.format(mode)
data = []
with open(path) as f:
    for line in f:
...
for data_entity in tqdm(data):
    ...
    #source_content 就是会议内容，target_content就是
...
def extract_oracle(samples):
    for idx, data_entity in samples:
        ...
        # Obtain oracle index
        with open('/home/zixuandeng_alan/Dyle_clean/QMSum/ALL/greedy/ref_{}/{}.ref'.format(mode, idx), 'w') as f:
            for sent in sent_tokenize(ref):
                print(sent, file=f)
    
        # Obtain gold summary reference
        with open('/home/zixuandeng_alan/Dyle_clean/QMSum/ALL/greedy/dec_{}/{}.dec'.format(mode, idx), 'w') as f:
            for sent in sent_tokenize(oracle):
                print(sent, file=f)

        # Obtain decoded oracle
        with open('/home/zixuandeng_alan/Dyle_clean/QMSum/ALL/greedy/index_{}/{}.dec'.format(mode, idx), 'w') as f:
...
eval_rouge('/home/zixuandeng_alan/Dyle_clean/QMSum/ALL/greedy/dec_{}'.format(mode), 
            '/home/zixuandeng_alan/Dyle_clean/QMSum/ALL/greedy/ref_{}'.format(mode))